    ...
    setProjects((prev) => ({
      ...
      [selectedProject.category]: prev[selectedProject.category].map((project) =>
        project.name === selectedProject.name
          ? { ...project, candidates: [...project.candidates, selectedCandidate] }
          : project
      ),
    }));
    ...
    <div style={{ padding: "24px", backgroundColor: "#f3f4f6", minHeight: "100vh" }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={10} style={{ padding: "20px", marginTop: "100px", marginBottom: "20px" }}>
          <h1 style={{ padding: "20px", marginTop: "20px" }}>Admin Dashboard</h1>
          <div
            style={{
              display: "grid",
          ...
          >
            {Object.keys(projects).map((category) => (
              <Card key={category} style={{ boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)", padding: "16px" }}>
                <CardContent>
                  <h2 style={{ fontSize: "18px", fontWeight: "600", color: "#374151", textTransform: "capitalize" }}>
                    {category} Projects
                  </h2>
                  {projects[category].map((project, index) => (
                    ...
                    >
                      <p style={{ color: "#4b5563" }}>
                        {project.name} ({project.candidates.join(", ")})
                      </p>
                      <Button
                        style={{ backgroundColor: "#4f46e5", color: "white" }}
                        onClick={() => handleAddCandidate(category, project.name)}
                      >
                        Manage
                      </Button>
                    </div>
          ...
          <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
            ...
            <DialogActions>
              <Button style={{ color: "#6b7280" }} onClick={() => setOpenDialog(false)}>
                Cancel
              </Button>
              <Button style={{ backgroundColor: "#4f46e5", color: "white" }} onClick={addCandidateToProject}>
                Assign
              </Button>
            </DialogActions>
          </Dialog>
